...
const basePath = process.env.CHATS_FOLDER
//------------------------------------------------------------------------------------------------------------------------
//Create list of folders
let foldersList = readdirSync(basePath)
let filteredFolderList = []
for (let folder of foldersList) {
  const result = JSON.parse(
    readFileSync(basePath.concat('\\', folder, '\\', 'message_1.json'), 'utf8')
  )
  if (result.messages.length > 1) {
    filteredFolderList.push(folder)
  }
}
//Iterate through every chat from the folder
for (let folder of filteredFolderList) {
  console.log(`Loading chat: ${folder}`)
  //Set folder path
  const folderPath = basePath.concat('\\', folder)
  //Get number of message files
  const nrMessage = readdirSync(folderPath).filter((fn) =>
    fn.includes('message_')
  ).length
  //Generate table name
  const fileSplit = folder.includes('_') ? folder.split('_') : ['t', folder]
  if (fileSplit[0] === 'facebookuser') {
    fileSplit[0] = 'f'
  }
  const tableName =
    fileSplit[0] === 't' || fileSplit[0] === 'f'
